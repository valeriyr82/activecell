    ...
    context "when there are some flash messages" do
      let(:flashes) do
        Hash.new.tap do |messages|
          ...
          messages[:empty] = ''
          messages[:error] = 'Houston, we have a problem!'
      ...
      it "should render a list with flash messages" do
        flashes.except(:empty).each do |type, message|
          should have_selector("div.alert.alert-#{type}", text: message)
        end
      end

      it "should not render empty messages" do
        should_not have_selector("div.alert alert-empty")
      end
    end
  end
