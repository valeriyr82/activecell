...
describe 'load_qbo_credit_card_credits' do
  include_context 'quickbooks online extraction setup', 'load_qbo_credit_card_credit'
  ...
  context 'without already existing financial transactions in the db' do
    ...
    it "should fill database with FinancialTransactions from response" do
      FinancialTransaction.count.should == 116
    end
    ...
    describe 'When looking at a single created transaction' do
      ...
      subject { FinancialTransaction.find_by(qbo_id: "1672-1", company_id: company.id, is_credit: true, amount_cents: -151401) }

      specify 'loaded transaction' do
        expect(subject.source).to eq('qbo_credit_card_credit')
        expect(subject.transaction_date).to eq(Date.parse("2012-03-08"))
  ...
  context 'with financial transactions in the db' do
    let!(:mentioned_transaction) { create(:financial_transaction, qbo_id: '1672-1', source: 'qbo_credit_card_credit',
    ...
    let!(:not_mentioned_transaction) { create(:financial_transaction, qbo_id: "someNonExistingId", account: account, 
        company: company, source: 'qbo_credit_card_credit', is_credit: true) }
    
    before { engine.process batch }

    it "should update an existing transaction" do
      mentioned_transaction.reload.amount_cents.should == -151401
    end
