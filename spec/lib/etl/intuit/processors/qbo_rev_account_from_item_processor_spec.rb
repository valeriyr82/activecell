...
describe ETL::Processor::QboRevAccountFromItemProcessor do
  
  describe "When :posting_type is :debit" do
    let!(:processor) {ETL::Processor::QboRevAccountFromItemProcessor.new(nil, {:posting_type => :debit})}
    let(:row) {{"ItemId" => 1, "RevAccountId" => 'thisShouldBeGone'}}
    
    before do
      @acc_lu = mock
      ETL::QBO.should_receive(:get_product_inc_account_lookup).and_return(@acc_lu)
    end
    
    describe "When account found by the first lookup" do
      before do
        @acc_lu.should_receive(:transform).with(nil, 1, row).and_return(123)
        ETL::QBO.should_not_receive :get_default_rev_account
      end
    ...
    describe "When account found by the second lookup" do
      before do
        ...
        ETL::QBO.should_receive(:get_default_rev_account).and_return(234)
      end
  ...
  describe "When :posting_type is :debit" do
    let!(:processor) {ETL::Processor::QboRevAccountFromItemProcessor.new(nil, {:posting_type => :credit})}
    let(:row) {{"ItemId" => 1, "RevAccountId" => 'thisShouldBeGone'}}
