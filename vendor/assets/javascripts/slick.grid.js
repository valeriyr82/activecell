...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    // modify
    if (options && options.modifier && options.modifier.replacements) {
        var toEval = "(function() {var view = options.modifier.viewInstance; ("
            + options.modifier.replacements.toString().replace('__scope.', '')
            + ").call(self)})()";
        eval(toEval);
    }
    // settings
    ...
    function createCssRules() {
      ...
      var rules = [
        "." + uid + " .slick-header-column { left: 1000px; height:" + options.rowHeight + "px;}",
    ...
    function resizeCanvas() {
      ...
      if(viewportH > options.maxTableHeight){
          $container.css('height', options.maxTableHeight + 'px')
          ...
          if (validationResults.valid) {
            if (activeRow < getDataLength()) {
              ...
              if (options.editCommandHandler) {
                ...
                editCommand.execute();
                makeActiveCellNormal();
                // to stay required css styles after edition
                $(activeCellNode).removeClass("active");
                $(rowsCache[activeRow].rowNode).removeClass("active");
              }
